 ...
 * Uses API for file operations instead of direct Node.js modules
 */
export async function executeWorkflow(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ message: 'Workflow execution failed' }));
    ...
    // Find the first markdown file to select by default
